...
import { getActiveContacts, type Contact } from '@/data/mockContacts'
...
const FloatingContactButtons = () => {
    ...
    const fetchContacts = async () => {
      try {
        ...
        if (response.success && response.data && Array.isArray(response.data) && response.data.length > 0) {
          ...
          console.warn('No contacts from API, using mock data')
          setContacts(getActiveContacts())
        }
      } catch (error) {
  ...
  const renderIcon = (contact: Contact) => {
    if (contact.iconType === 'image' && contact.icon) {
      return <img src={contact.icon} alt={contact.name} className="w-6 h-6" />
    }
    
    // Default phone icon for phone type
    if (contact.type === 'phone') {
      return (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <stop stopColor="#00652E"/>
          <stop offset="1" stopColor="#00CB5C"/>
          </linearGradient>
    ...
    return <span className="text-white text-sm font-bold">{contact.label.charAt(0)}</span>
  }
  ...
  if (contacts.length === 0) {
    return null // Don't show if no contacts
  }
          ...
          {contacts.map((contact, index) => (
            ...
            >
              ...
              <a
                ...
                className={`border-2 border-[#00652E]  p-2 rounded-2xl hover:scale-110 transition-all duration-200 transform flex items-center justify-center`}
